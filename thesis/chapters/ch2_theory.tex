...
Most results in this chapter come from the cited literature and are independently re-derived and checked numerically. Two results are original to this thesis: the correction to the natural-parameter inverse (Remark~\ref{rem:gjtypo}) and the endpoint-certification result (Theorem~\ref{lem:range}). Results from the literature are labelled \emph{Proposition} or \emph{Lemma}, while \emph{Theorem} is reserved for original results. Longer verification proofs are given in Appendix~\ref{app:proofs}.
...
appendix: it is the derivation that supplies the term missing from the published inverse
(Remark~\ref{rem:gjtypo}).
...
\begin{remark}[\textbf{A correction to the published inverse}]\label{rem:gjtypo}
During the replication, we found that the natural-to-raw inverse reported in
\cite[Lemma~3.2]{gatheraljacquier2014} is missing the term
$-b\sigma\sqrt{1-\rho^2}$ in $\Delta$, or equivalently
$-\alpha\sqrt{1-\rho^2}$ in the notation of the paper. This term appears directly in the derivation above.

The omission matters in practice because any calibration that passes through the natural parameterisation produces an incorrect value of $\Delta$, and therefore an incorrect SVI slice. We identified the issue by testing $2{,}000$ random parameter sets through the cycle raw$\to$natural$\to$JW$\to$raw. With the corrected term, the round trip closes with a maximum error of $2.7\times10^{-10}$. 
\end{remark}


\subsection{The Jump-Wings parameterisation}

The raw and natural parameterisations are useful for calibration but they are not very intuitive: a set such as $(a,b,\rho,m,\sigma)$ does not immediately tell us what the smile looks like. The \emph{Jump-Wings} (JW) parameterisation \cite{gatheraljacquier2014} describes the same slice using five quantities that a trader can read directly from the smile. At maturity $t$, with $w_t:=v_t,t$ the ATM total variance, these are the ATM variance $v_t$ and ATM skew $\psi_t$, which describe the level and slope of the smile at the money; the left and right wing slopes $p_t$ and $c_t$, which describe the steepness of the put and call wings; and the minimum implied variance $\tilde v_t$ may be shifted away from the money when $\psi_t\neq0$.
