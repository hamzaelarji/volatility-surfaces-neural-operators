...
raw$\to$natural$\to$JW$\to$raw. The largest reconstruction error is $2.7\times10^{-10}$.
...
\end{equation}
to the reported precision (Notebook~02). We then reproduce the repair procedure
...
Figure~\ref{fig:vogt} from $\min_k g=-0.0329$ to $+0.2704$ on the audit range $k\in[-2,2]$, thereby removing the
region of negative implied density; on the repaired slice the minimum sits at the edge of the
range, the interior being uniformly positive (Notebook~02).
...
Median crossedness          & $2.07\times10^{-2}$ & $0$ & $1.14\times10^{-2}$ & $0$ \\
\bottomrule
\end{tabular}\\[2pt]
\end{table}
...
\cite{ackerer2020}. With ten nodes on the synthetic experiment's maturity axis $[0.06,1.4]$
there is \emph{no node at all} between $0.987$ and $1.400$, a $0.41$-year hole covering $31\%$
of the axis
...
\paragraph{Repair.} The butterfly penalty keeps the exponential maturity grid, dense at the short
end where that constraint bites; the calendar penalty, which costs only one first derivative per node, gets its own uniform grid of
...
2018-01-08, which carries six butterfly-violating SVI slices with $\min_k g=-0.072$. On these
days the smoother returns zero strict violations on both domains, no blind flags, and a valid
certificate every day, with a hold-out error of $1.15$--$1.31\,\vp$ under the endpoint-certified
calibrator of Remark~\ref{rem:cert}. That calibrator is the one behind every deep-smoother
number in this thesis, including the surface packs consumed in Chapter~\ref{ch:downstream}. An
earlier calibrator that clipped $\gamma\le\tfrac12$ returned $1.70$--$1.81\,\vp$ on the same
days; the episode was diagnosed here, the per-day prior-only RMSE being what localised it, and
endpoint certification recovers the free fit.
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{%
        \includegraphics[width=0.9\textwidth]{figures/nb04_c09_2.png}
    }
...
hull-restricted in R1 and $21.9\%$ in R3, essentially unchanged. If anything the restricted rate
is marginally higher than the full-box rate, because the violating cells lie mainly inside the
quoted put wing (Figure~\ref{fig:nb04audit}) rather than in the unquoted corners of the box: the
failure is not an extrapolation artefact.
...
GNO      & R3 pre+fine   & $\mathbf{1.26}$ & $21.75\%$ & $0.067\%$ & 355 & 86 & $-32.6$ & 386 \\
\bottomrule
\end{tabular}\\[2pt]
...
from $2.69$ to $1.32\,\vp$, closing almost all of the gap to the accuracy leader, while both
audit columns go
